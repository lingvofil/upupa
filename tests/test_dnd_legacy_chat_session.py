...
def test_legacy_gemini_chat_is_created_on_first_direct_send(monkeypatch):
    ...
    monkeypatch.setattr(dnd.model, "start_chat", start_chat)
...
def test_rewind_rebuilds_legacy_chat_only_if_it_was_already_materialized(monkeypatch):
    starts = []
    old_chat = object()
    rebuilt_chat = object()
    session = SimpleNamespace(
