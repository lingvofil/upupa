...
async def generate_insult_for_lis(message, model_instance):
    ...
    prompt = (
        "Составь колкую фразу в ответ, используя или склоняя, иногда вставляй закрывающую скобку, "
        "сочетая любые из этих слов и фраз, меняя их падежи и формы, "
...
async def generate_reaction_for_113086922(message: Message, model_instance):
    ...
    """
    try:
        if random.random() < 0.5: # 50% шанс сгенерировать новую фразу (микс)
