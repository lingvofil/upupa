...
def _edge_instruction(edge: GraphEdge, names: dict[int, str]) -> str:
    ...
    if edge.reciprocity >= 0.72:
        return (
            ...
            "оба активно тянут к себе облачка сообщений, как два идиота с рациями"
        )
    ...
    if high > 0 and low / high <= 0.35:
        ...
        return (
            ...
            f'у "{source}" нелепый мегафон, а "{target}" слегка офигел; обратная стрелка почти незаметна'
        )
...
def build_cringe_social_graph_prompt(view: RenderGraph, period_days: int) -> str:
    ...
    return f"""Нарисуй намеренно ВСРАТУЮ юмористическую схему взаимодействий участников Telegram-чата за последние {period_days} дней.
Это визуальная шутка по реальной статистике сообщений, реплаев, упоминаний и реакций. Не придумывай любовь, дружбу, секс, вражду или реальные личные отношения: показывай только интенсивность и направленность общения.

СТИЛЬ — КРИТИЧЕСКИ ВАЖНО:
- будто человек в 2007 году впервые открыл MS Paint и за три минуты рисовал мышкой;
- очень кривые примитивные карикатурные рожи, палочные руки и ноги, плохая анатомия, тупые выражения лиц;
- неровные линии разной толщины, уродские стрелки, нелепые облачка сообщений, молнии, каракули и дешёвый клипарт;
...
УЧАСТНИКИ. Каждый должен быть изображён ровно один раз отдельной кривой карикатурой:
{participant_lines}
