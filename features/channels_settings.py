...
NEURORECIPES_COMMAND = "нейрорецепты"
...
async def process_channel_command(message: types.Message, channel_settings: dict):
    ...
    if command == "каламбур" and "urls" in channel_info:
        ...
        prepared_info = dict(channel_info)
        logging.info(f"Обрабатываем команду '{command}'. URL: {prepared_info.get('url')}")

    # Для нейрорецептов к картинке добавляем только первую строку текста
    # того же поста, из которого была взята картинка.
    prepared_info["caption_from_post_title"] = command == NEURORECIPES_COMMAND
...
def _clean_post_title(value):
    """Return the first non-empty post line suitable for a Telegram caption."""
...
async def _process_random_media(message: types.Message, channel_info: dict) -> bool:
    ...
    while attempt < max_attempts:
        try:
            ...
            caption = None
            if channel_info.get("caption_from_post_title"):
                caption = _clean_post_title(media_item.get("post_text"))
...
async def _download_random_media(url):
    """Downloads a random media file from a given URL together with its post text."""
    try:
            ...
            try:
                ...
                media_urls = await page.evaluate("""
                    () => {
                        ...
                        const postTextFor = (media) => {
                            const post = media.closest(
                                '.post-container, .post-item, .channel-post, article, [class*="post-container"], [class*="post_item"]'
                            );
                            return firstUsefulText(post);
                        };
