...
CI запускается для pull request и push в `refactor`; при push в `main` тот же workflow вызывается из деплоя до подключения к серверу. Он собирает coverage для основных пакетов, запускает `pyflakes` для архитектурного ядра (`main.py`, `app`, `core`, `infrastructure`) и держит минимальный coverage floor **30%**.
...
Сначала reusable workflow `tests.yml` проверяет точный SHA этого push: static checks, тесты и coverage floor. Задача деплоя зависит от успешного результата через `needs: test`; при ошибке, отмене или пропуске тестов подключение к VPS не запускается. Отдельного повторного запуска тестов на push в `main` нет.
...
5. health-check с 12 попытками проверяет systemd, готовность процесса бота через локальный `/ready` (polling, SQLite, фоновые циклы, совпадение PID) и Telegram `getMe`;
6. при ошибке workflow откатывает код и зависимости на предыдущий commit; перед возвратом на старую JSON-версию останавливает бот и экспортирует актуальные счётчики. Pre-deploy backup сохраняется.

Счётчики сообщений и ранги хранятся в таблице `rank_counters` в `statistics.db`. Первый запуск автоматически импортирует `message_stats.json` один раз, сохраняя исходный файл. Ошибка формата останавливает запуск без обнуления данных. Локальная диагностика: `python scripts/production_healthcheck.py --app-dir /path/to/upupa`; порт по умолчанию `8766`, настройка — `UPUPA_HEALTHCHECK_PORT` (одинаковая для сервиса и диагностики).

История индексируется в `history.db` (SQLite + FTS5). Деплой после backup подготавливает индекс до переключения кода, пока старый бот продолжает работать; при старте импортируется оставшийся хвост. При ручном запуске первый импорт выполняется на startup. Прогресс сохраняется порциями между попытками. Сводки, радио, поиск обсуждений и профили читают индекс. `user_messages.log` сохраняется для восстановления и работы старой версии при откате; вручную обрезать его нельзя. Подробности — в [ARCHITECTURE.md](ARCHITECTURE.md#индексированная-история).

Сводка имеет два режима с общим промптом:

- `чобыло` — последние 12 часов при каждом запросе.
- `что я пропустил` / `что я пропустила` — сообщения после последней успешно доставленной тебе сводки в этом чате; первый запрос охватывает 12 часов. Поддерживается префикс `упупа`.

Оба режима обновляют персональную отметку только после полной отправки ответа. Это отметка сводки, а не прочтения сообщений в Telegram. Ошибка AI/отправки не сдвигает её; сообщения, пришедшие во время генерации, остаются для следующего запроса. Если новых сообщений нет, AI не вызывается. В группе ответ виден всем, но отметка меняется только у инициатора. Для анонимного отправителя доступно только `чобыло`. Отметки хранятся в `history.db` и переживают перезапуск. Большой период, как и в `чобыло`, суммируется по ограниченной выборке до 5000 сообщений, включая последние 1000; размер выборки показан перед ответом.
