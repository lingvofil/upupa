...
SCENE_MOMENTUM_RULES = (
    "СМЕНА ДЕКОРАЦИЙ НЕ РАВНА ПОСТОЯННОМУ ЭКШЕНУ. Сравни ближайшую сцену с 2–3 предыдущими. Если партия уже два эпизода "
    "подряд находится в одном типе пространства или решает один вид проблемы, при ближайшей естественной возможности "
    "измени место, масштаб, цель или окружение — но не обязана добавлять новую угрозу. Спокойная прогулка по рынку, осмотр "
    "места, разговор с NPC, торговля, поиск слухов, дорога, отдых и бытовое взаимодействие — полноценные сцены. После "
    "напряжённого эпизода обычно дай хотя бы одну сцену с низким давлением, где герои сами выбирают темп. Погоня, пожар, "
    "обвал, шторм, дедлайн, нападение и другая срочность появляются только из уже установленной причинности или действий "
    "игроков, а не ради того, чтобы «что-то происходило». Не превращай исследовательское действие вроде «осматриваюсь», "
    "«ем яблоко» или «разговариваю со старостой» в повод для новой случайной катастрофы. Чередуй социальные, исследовательские, "
    "бытовые, загадочные и боевые эпизоды. Роботы, дроны, киберживотные, стража и одинаковые монстры не являются универсальным "
    "источником конфликта. Новая декорация должна вытекать из действий игроков или сюжета, а не быть случайным телепортом."
)

OPENING_EXPLORATION_RULES = (
    "ПЕРВЫЕ СЦЕНЫ ПАРТИИ — ОРИЕНТАЦИЯ. Сначала дай героям освоиться в месте: увидеть детали, поговорить с NPC, осмотреться, "
    "потрогать предметы и заявить собственные намерения. Первая сцена режима с участниками должна заканчиваться ОБЩИМ "
    "ACTION:INPUT без TARGETS, чтобы каждый герой один раз решил, что делает. После первого общего круга обязательно явно "
    "учти действие каждого героя и только постепенно своди их последствия в общую канву. Не запускай голосование, драку, "
    "погоню, катастрофу или срочный дедлайн до этого круга просто ради темпа. Если действия игроков сами создают риск, "
    "разрешай этот риск честно; иначе оставляй пространство для ещё одного INPUT, разговора или исследования."
)

PACING_RULES = (
    "Темп партии — это чередование напряжения и свободы, а не постоянное ускорение. Между двумя решениями игроков допускай "
    "не более одной непосредственной атаки NPC. После неё верни инициативу партии и дай спокойно отреагировать. Не создавай "
    "новые волны врагов, новые аварии или новый дедлайн только чтобы продлить сцену. Схватка должна продвигать сюжет и быстро "
    "заканчиваться; исследование, разговор и обычный свободный ход не обязаны перерастать в схватку. Сначала разрешай то, что "
    "игроки реально заявили, и только затем добавляй новое внешнее событие. Решение, выбранное игроками в только что завершённом "
...
def _force_story_start_group_input(response: str) -> str:
    """Give every participant one free opening decision before escalation."""
    text = str(response or "").strip()
    if _ACTION_RE.search(text):
        return _ACTION_RE.sub("[ACTION:INPUT]", text, count=1)
    return (text + "\n[ACTION:INPUT]").strip()
...
def install_dnd_pacing(dnd) -> None:
    ...
    def campaign_context(dnd_module, session):
        result = (
            original_context(dnd_module, session)
            + _recent_scene_context(session)
            + "\nТЕМП И ПРИЧИННОСТЬ:\n"
            + PACING_RULES
        )
        if dnd_module._is_participant_mode(session) and int(getattr(session, "scene_count", 0) or 0) <= 2:
            result += "\n\n" + OPENING_EXPLORATION_RULES
        return result
    ...
    async def parse_turn(bot, chat_id, response):
        ...
        if session:
            ...
            if dnd._is_participant_mode(session) and source_kind == "STORY_START":
                guarded_start = _force_story_start_group_input(response)
                if guarded_start != str(response or ""):
                    logging.info("DnD forced opening group INPUT chat_id=%s", chat_id)
                response = guarded_start
