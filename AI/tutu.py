...
SESSION_URL = "https://avia.tutu.ru/session"
...
async def create_search_session(client: httpx.AsyncClient, payload: Dict[str, Any]) -> Optional[str]:
    response = await client.post(SESSION_URL, json=payload, timeout=15.0)
    response.raise_for_status()
    data = response.json()
...
async def search_tickets(params: SearchParams) -> List[str]:
    messages = []
    async with httpx.AsyncClient(headers=DEFAULT_HEADERS, http2=True) as client:
