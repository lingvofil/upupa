...
ДЛИНА: обычно 100–180 слов, ориентир до 220 слов художественного текста.
Покажи конкретное последствие решения, выразительную деталь и реакцию мира в двух-трёх абзацах.
Простое действие можно разрешить короче. Не пересказывай прошлые ходы вместо текущего.
При нескольких заявках сохрани последствия каждого героя. Заверши предложения и вопрос следующему игроку.

АДРЕСНЫЕ СВОБОДНЫЕ ХОДЫ — ОСНОВА ИГРЫ. ACTION:INPUT можно ставить несколько эпизодов подряд,
...
В режиме участников обращайся по имени к одному конкретному герою: «Что ты делаешь?» и указывай его TARGETS.
Общий ход нужен лишь для совместного плана или общей развилки, ориентир — один на 4–6 личных ходов.
...
def _compact_request_text(text: str) -> str:
    ...
    result = result.replace(
        "СТРОГО до 100 слов",
        "обычно 100–180 слов, ориентир до 220 слов",
    )
    result = result.replace(
        "не более 100 слов",
        "обычно 100–180 слов, ориентир до 220 слов",
...
async def _generate_without_consecutive_input(original_generate, session, prompt: str) -> str:
    ...
    if action_count:
        prepared_request += (
            ...
            "Сохрани конкретные последствия КАЖДОЙ заявки, не обрывай сцену ради лимита."
        )
    raw_result = await original_generate(session, prepared_request)
    # Never discard already-authored consequences or metadata to meet a style
    # budget. Length belongs in the generation request, not a destructive slice.
...
def configure_dnd_style() -> None:
    """Install only DnD presentation/style wrappers. Safe to call more than once."""
    from AI import dnd
