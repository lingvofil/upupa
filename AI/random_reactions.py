...
async def generate_situational_reaction(chat_id: int):
    ...
    prompt = f"""
    Проанализируй диалог из чата. Придумай короткую, кинематографичную ремарку или звуковой эффект, который органично дополняет ситуацию.
    Ремарка должна быть естественной, нейтральной по тону и предсказуемой для контекста.
    Не используй абсурд, жесткие оскорбления и шок-контент.
    Ответь ТОЛЬКО ОДНОЙ фразой, курсивом, заключенной в звездочки (*).

    Примеры ремарок:
    - *в комнате на секунду стало тихо*
    - *кто-то сдержанно усмехнулся за кадром*
    - *повисла неловкая пауза*
    - *на фоне слышен короткий вздох*
    - *в чате стало заметно спокойнее*
    ...
    try:
        reaction_text = await generate_with_model(prompt, chat_id, temperature=0.45, max_tokens=60)
