...
POLL_AGENCY_RULES = f"""
{POLL_AGENCY_MARKER}.
Используй ACTION:POLL только для настоящей ОБЩЕЙ закрытой развилки, где несколько заранее сформулированных
вариантов действительно исчерпывают очевидный выбор. Любой конкретный путь, который ты называешь в художественном
...
Если естественных решений много, герои могут импровизировать или ты не уверен, что список полный — используй ACTION:INPUT,
а не POLL. Runtime добавляет к общему голосованию «Свой вариант» как аварийный выход из фиксированного меню.
""".strip()
