...
async def handle_pun_image_command(message: types.Message):
    ...
    try:
        ...
        pun_prompt = (
            ...
            "Хорошие примеры:\n"
            "нарвал+валик = нарвалик (общий слог «вал»)\n"
            "фета+такса = фетакса (общий слог «та»)\n"
            "покемон+мона лиза = покемона лиза (общий слог «мон»)\n"
            "кабан+банан = кабанан (общий слог «бан»)\n"
            "страус+аустралия = страустралия (общий слог «ауст»)\n"
            "Плохие примеры (не делай так): кот+лампа = котлампа, бутерброд+карандаш = бутеркарандаш — "
            "это просто склейка, не каламбур.\n"
            "Выбирай неожиданные и смешные сочетания, желательно визуально абсурдные.\n"
...
async def generate_gradio_img2img(image_bytes: bytes, prompt: str) -> Optional[bytes]:
    def _call_gradio() -> Optional[bytes]:
        client = Client("victor/dlss-5-anything", token=HF_TOKEN)
...
async def handle_redraw_command(message: types.Message):
    ...
    try:
        ...
        final_prompt = (
            "drawn in extremely bad children's crayon style,"
            "wobbly lines, wrong proportions, messy coloring outside the lines,"
            "crude doodle on white paper, drawn by a 4 year old,"
            f"{description}"
        )
