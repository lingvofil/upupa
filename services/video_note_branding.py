...
def _outside_circle_mask(size: int) -> Image.Image:
...
def _draw_arc_text(layer: Image.Image, size: int) -> None:
    """Draw the bot handle along the lower-right outside arc."""
...
def build_branding_plate(
    *,
    ...
    """Create an RGBA overlay whose entire inscribed circle stays transparent."""
    ...
    outside_mask = _outside_circle_mask(size)
    artwork = Image.new("RGBA", (size, size), (18, 18, 20, 255))

    icon_size = max(48, round(size * 0.145))
    icon = _round_mascot(mascot, icon_size)
    icon_x = max(2, round(size * 0.010))
    icon_y = size - icon_size - max(2, round(size * 0.010))
    ...
    return Image.composite(artwork, transparent, outside_mask)
